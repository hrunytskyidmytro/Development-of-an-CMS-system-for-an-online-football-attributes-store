...
class Clothing extends Model
{
    ...
    public function ChangePhoto($id, $file)
    {
        ...
        if(is_file($folder.$clothing['photo'].'_b.jpg') &&  is_file($folder.$file))
            unlink($folder.$clothing['photo'].'_b.jpg');

        if(is_file($folder.$clothing['photo'].'_m.jpg') &&  is_file($folder.$file))
            unlink($folder.$clothing['photo'].'_m.jpg');

        if(is_file($folder.$clothing['photo'].'_s.jpg') &&  is_file($folder.$file))
            unlink($folder.$clothing['photo'].'_s.jpg');
        $clothing['photo'] = $file_path['filename'];
        ...
        $this->UpdateClothing($clothing, $id);
    }
     ...
     * @return array|false
     */
    public function GetLastClothing($count)
    {
        return \core\Core::getInstance()->getDB()->select('clothing','*', null,['datetime' => 'DESC'], $count);
    }
    ...
    {
        $clothing = \core\Core::getInstance()->getDB()->select('clothing','*', ['id' => $id]);
        if(!empty($clothing))
            return $clothing[0];
        else
            return null;
     ...
     * @return array|bool
     */
    public function AddClothing($row)
    {
        ...
        $id = \core\Core::getInstance()->getDB()->insert('clothing',$rowFiltered);
        return [
            'error' => false,
            'id' => $id
        ];
     ...
     * Редагування товару
     */
    public function UpdateClothing($row, $id)
    {
        ...
        \core\Core::getInstance()->getDB()->update('clothing', $rowFiltered, ['id' => $id]);
        return true;
    }
    ...
    public function DeleteClothing($id)
    {
        ...
        $user = $userModel->GetCurrentUser();
        if(empty($clothing) || empty($user) || $user['id'] != $clothing['user_id'])
            return false;
        \core\Core::getInstance()->getDB()->delete('clothing', ['id' => $id]);
        return true;
    }
    ...
    {
        $comments = \core\Core::getInstance()->getDB()->select('response_for_clothing', '*',['tovar_id' => $id]);
