...
class ResponseForFootballShoes extends Model
{
    /**
     * Додавання відгуку
     * @param $row
    ...
    public function AddResponse($row)
    {
        ...
        $footballShoesModel = new \models\FootballShoes();

        $foot = $footballShoesModel->GetFootballShoe($_GET['id']);
        ...
        $id = \core\Core::getInstance()->getDB()->insert('response_for_football_shoes',$rowFiltered);
        return [
            'error' => false,
            'id' => $id
        ];
    ...
    {
        return \core\Core::getInstance()->getDB()->select('response_for_football_shoes','*', null,['datetime' => 'DESC'], $count);
    }
    ...
    {
        $response = \core\Core::getInstance()->getDB()->select('response_for_football_shoes','*',['id' => $id]);
        if(!empty($response))
            return $response[0];
        else
            return null;
    ...
    public function UpdateResponse($row, $id)
    {
        ...
        \core\Core::getInstance()->getDB()->update('response_for_football_shoes', $rowFiltered, ['id' => $id]);
        return true;
    }
    ...
    public function DeleteResponse($id)
    {
        ...
        if(empty($response) || empty($user) || $user['id'] != $response['user_id'])
            return false;
        \core\Core::getInstance()->getDB()->delete('response_for_football_shoes', ['id' => $id]);
        return true;
    }
