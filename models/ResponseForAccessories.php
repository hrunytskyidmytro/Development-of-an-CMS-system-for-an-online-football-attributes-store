...
class ResponseForAccessories extends Model
{
    /**
     * Додавання відгуку
     * @param $row
    ...
    public function AddResponse($row)
    {
        ...
        $accessoriesModel = new \models\Accessories();

        $foot = $accessoriesModel->GetAccessories($_GET['id']);
        ...
        $id = \core\Core::getInstance()->getDB()->insert('response_for_accessories',$rowFiltered);
        return [
            'error' => false,
            'id' => $id
        ];
    ...
    {
        return \core\Core::getInstance()->getDB()->select('response_for_accessories','*', null,['datetime' => 'DESC'], $count);
    }
    ...
    {
        $response = \core\Core::getInstance()->getDB()->select('response_for_accessories','*',['id' => $id]);
        if(!empty($response))
            return $response[0];
        else
            return null;
    ...
    public function UpdateResponse($row, $id)
    {
        ...
        \core\Core::getInstance()->getDB()->update('response_for_accessories', $rowFiltered, ['id' => $id]);
        return true;
    }
    ...
    public function DeleteResponse($id)
    {
        ...
        if(empty($response) || empty($user) || $user['id'] != $response['user_id'])
            return false;
        \core\Core::getInstance()->getDB()->delete('response_for_accessories', ['id' => $id]);
        return true;
    }
    /**
